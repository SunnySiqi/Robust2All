...
def train(test_envs, args, hparams, n_steps, checkpoint_freq, logger, writer, target_env=None):
    ...
    print("TEST ENVS in TRAIN!!!!!", test_envs)
    ...
    #save_loss_dict = {"elr_loss":[], "miro_loss":[]}
    for step in range(n_steps):
        ...
        #print("batches keys!!!", batches.keys())
        #inputs = {**batches, "step": step}
        inputs = {**batches, "step": step, 'env_list': train_envs}
        step_vals = algorithm.update(**inputs)
        for key, val in step_vals.items():
            checkpoint_vals[key].append(val)
        # for key, val in step_vals.items():
        #     if key in ["loss", "reg_loss"]:
        #         checkpoint_vals[key].append(val)
        #     else:
        #         save_loss_dict[key].append(val)
        checkpoint_vals["step_time"].append(time.time() - step_start_time)

        # del step_vals['elr_loss']
        # del step_vals['miro_loss']
        ...
        if step % checkpoint_freq == 0:
            # save losses:
            # for key, val in save_loss_dict.items():
            #     with open('loss_output/'+'fashion_'+key+'.pkl', 'wb') as f:
            #         pickle.dump(val, f)
