...
import argparse
import numpy as np
import dataloader_dg_cp as dataloader
...
parser.add_argument('--lr', '--learning_rate', default=5e-4, type=float, help='initial learning rate')   ## Set the learning rate to 0.005 for faster training at the beginning
...
parser.add_argument('--data_path', default='../../baseline_fine/FINE_official-master/dynamic_selection/dir_to_data/CP/', type=str, help='path to dataset')
parser.add_argument('--seed', default=123)
parser.add_argument('--gpuid', default=0, type=int)
parser.add_argument('--pretrained', default=True, type=bool)
parser.add_argument('--num_class', default=4, type=int)
parser.add_argument('--dataset', default="CP", type=str)
parser.add_argument('--resume', default=False, type=bool, help = 'Resume from the warmup checkpoint')
parser.add_argument("--algorithm", type=str, default="MIRO")
parser.add_argument("--swad", action="store_true")
parser.add_argument("--train_domain", default=["Train", "Task_one", "Task_two"], type=str, nargs="*", help="training subsets")
parser.add_argument("--test_domain", default=["Task_three"], type=str,nargs="*", help="testing subsets")
...
## Training
def train(epoch, net, net2, labeled_trainloader, unlabeled_trainloader, swad_net=None):
...
def val(net,val_loader,k, swad=None):
    ...
    with torch.no_grad():
        for batch_idx, (inputs, targets) in enumerate(val_loader):
            ...
            loss_sum += basic_loss*targets.size(0)
...
loader = dataloader.dg_CP_dataloader(root=args.data_path, batch_size=args.batch_size, warmup_batch_size = args.batch_size*2, num_workers=5, test_domain=args.test_domain[0], train_domain=args.train_domain, save_file=args.save_data_file_name, use_domainlabels=False)
print('| Building Net')
...
# model_name_1 = 'dg_cp_net1.pth.tar'
# model_name_2 = 'dg_cp_net2.pth.tar'
...
acc_meter = torchnet.meter.ClassErrorMeter(topk=[1,3], accuracy=True)
nb_repeat = 2
...
for epoch in range(0, args.num_epochs+1):   
    ...
    if epoch<warm_up:             
        ...
        print('Warmup Net1')
        warmup(net1,train_loader)
        
        print('\nWarmup Net2')
        train_loader = loader.run(0,'warmup')
        warmup(net2,train_loader)
        ...
        for i in range(nb_repeat):
            ...
            if args.swad:
                train(epoch, net1, net2,labeled_trainloader, unlabeled_trainloader, swad_net1)
                swad1.update_and_evaluate(swad_net1, last_acc1)
                acc1 = val(net1,val_loader,1, swad1)
                last_acc1 = acc1
            else:
                train(epoch, net1, net2,labeled_trainloader, unlabeled_trainloader)                        ## Train Net1
        ...
        for i in range(nb_repeat):
            ...
            if args.swad:
                train(epoch,net2,net1,labeled_trainloader, unlabeled_trainloader, swad_net2)
                swad2.update_and_evaluate(swad_net2, last_acc2)
                acc2 = val(net2,val_loader,2, swad2)
                last_acc2 = acc2 
            else:
                train(epoch,net2,net1,labeled_trainloader, unlabeled_trainloader)                             ## Train net2
