...
import argparse
import numpy as np
import dataloader_dg_clothing as dataloader
...
def Calculate_JSD(epoch,model1, model2):
    ...
    for batch_idx, (inputs, targets, path) in enumerate(eval_loader):
        ...
        with torch.no_grad():
            ...
            out2 = torch.nn.Softmax(dim=1).cuda()(model1.predict(inputs)[1])
...
loader = dataloader.dg_clothing_dataloader(root=args.data_path, batch_size=args.batch_size, warmup_batch_size = args.batch_size*2, num_workers=8, num_batches=args.num_batches)
print('| Building Net')
...
model_name_1 = 'dg_clothing_net1.pth.tar'
model_name_2 = 'dg_clothing_net2.pth.tar'
