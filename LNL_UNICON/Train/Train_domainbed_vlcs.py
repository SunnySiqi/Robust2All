...
import argparse
import numpy as np
import dataloader_dg_vlcs as dataloader
import dataloader_vlcs_domainlabels as domain_label_dataloader
...
from domainbed.algorithms.miro import MIRO
...
parser.add_argument('--data_path', default='/research/swang/research/Dataset/VLCS', type=str, help='path to dataset')
...
parser.add_argument("--train_domain", default=["LabelMe", "SUN09", "VOC2007"], type=str, nargs="*", help="training subsets")
parser.add_argument("--test_domain", default=["Caltech101"], type=str,nargs="*", help="testing subsets")
parser.add_argument("--save_data_file_name", type=str, default="vlcs.npz")
parser.add_argument("--dataloader_w_domainlabels", default=False, type=bool)

args = parser.parse_args()

hparams = hparams_registry.default_hparams(args.algorithm, args.dataset)
keys = ["domainbed/config.yaml"]
...
## Training
def train(epoch, net, net2, optimizer, labeled_trainloader, unlabeled_trainloader, swad_net=None):
    ...
    for batch_idx, (inputs_x, inputs_x2, inputs_x3, inputs_x4, labels_x, w_x) in enumerate(labeled_trainloader):      
        ...
        with torch.no_grad():
            ...
            # _, outputs_u11 = net.predict(inputs_u3)
            # _, outputs_u12 = net.predict(inputs_u4)
            # _, outputs_u21 = net2.predict(inputs_u3)
            # _, outputs_u22 = net2.predict(inputs_u4)         
            ...
            # _, outputs_x  = net.predict(inputs_x3)
            # _, outputs_x2 = net.predict(inputs_x4) 
        ...
        # f1, _ = net.predict(inputs_u)
        # f2, _ = net.predict(inputs_u2)
        f1, _ = net(inputs_u)
        f2, _ = net(inputs_u2)
        ...
        #_, logits = net.predict(mixed_input)
        _, logits = net(mixed_input)
        #_, _ = net.update(mixed_input, mixed_target)
        ...
        #_, _ = net.update(mixed_input, mixed_target) #miro
        if args.swad:
            swad_net.update_parameters(net, step=batch_idx*args.batch_size)
...
def warmup(net,optimizer,dataloader):
    ...
    for batch_idx, (inputs, labels, path) in enumerate(dataloader):      
        ...
        #_ , outputs = net.predict(inputs)
        # _, _ = net.update(inputs, labels)              
...
# def val(net,val_loader,k, swad=None):
#     #net.eval()
#     if args.swad:
#         net = swad.get_final_model()
#     correct = 0
#     total = 0
#     with torch.no_grad():
...
def val(net,val_loader,k, swad=None):
    ...
    dist_loss_sum = 0
    with torch.no_grad():
        for batch_idx, (inputs, inputs_strong, targets) in enumerate(val_loader):
            inputs, inputs_strong, targets = inputs.cuda(), inputs_strong.cuda(), targets.cuda()
            if args.swad:
                outputs = net.predict(inputs)
            else:
                # _, outputs = net.predict(inputs)
                # _, outputs_strong = net.predict(inputs_strong)
                _, outputs = net(inputs)
                _, outputs_strong = net(inputs_strong)
            _ , predicted = torch.max(outputs, 1)
            _ , predicted_strong = torch.max(outputs_strong, 1)
            prob1 = F.softmax(outputs, dim=1)
            prob2 = F.softmax(outputs_strong, dim=1)          
                       
            total += targets.size(0)
            correct += predicted.eq(targets).cpu().sum().item()
            basic_loss = CEloss(prob1, targets)
            aug_loss = CEloss(prob2, targets)
            dist_loss = F.kl_div((prob2+1e-8).log(), (prob1+ 1e-8)).item()
            loss = 0.5*basic_loss + 500*dist_loss
            loss_sum += loss*targets.size(0)
            basic_loss_sum += basic_loss*targets.size(0)
            aug_loss_sum += aug_loss*targets.size(0)
            dist_loss_sum += dist_loss*targets.size(0)

    acc = 100.*correct/total
    avg_loss = 1.0*loss_sum/total
    #print("\n| Validation\t Net%d  Acc: %.2f%%, Loss: %.2f%% " %(k,acc, avg_loss))
    print("\n| Validation\t Net%d  Acc: %.2f%%, Basic Loss: %.2f , Aug Loss: %.2f, Distance Loss: %.2f , Sum Loss: %.2f" %(k,acc, 1.0*basic_loss_sum/total, 1.0*aug_loss_sum/total, 1.0*dist_loss_sum/total, avg_loss))    
    if avg_loss < best_loss[k-1]:
        best_loss[k-1] = avg_loss
        print('| Saving Best Net%d ...'%k)
        save_point = os.path.join(model_save_loc, '%s_net%d.pth.tar'%(args.id,k))
        torch.save(net.state_dict(), save_point)
    return acc

def test(net1,net2,test_loader, swad1=None, swad2=None):
    acc_meter.reset()
    net1.eval()
    net2.eval()
    if args.swad:
        net1 = swad1.get_final_model()
        net2 = swad2.get_final_model()
    correct = 0
    total = 0
    with torch.no_grad():
        for batch_idx, (inputs, targets) in enumerate(test_loader):
            inputs, targets = inputs.cuda(), targets.cuda()
            if args.swad:
                outputs1  = net1.predict(inputs)       
                outputs2  = net2.predict(inputs) 
            else:
                # _, outputs1  = net1.predict(inputs)       
                # _, outputs2  = net2.predict(inputs) 
...
def Calculate_JSD(epoch,model1, model2):
    ...
    for batch_idx, (inputs, targets, path) in enumerate(eval_loader):
        ...
        with torch.no_grad():
            # out1 = torch.nn.Softmax(dim=1).cuda()(model1.predict(inputs)[1])     
            # out2 = torch.nn.Softmax(dim=1).cuda()(model2.predict(inputs)[1])
...
if args.dataloader_w_domainlabels:
    loader = domain_label_dataloader.dg_VLCS_domainlabel_dataloader(root=args.data_path, batch_size=args.batch_size, warmup_batch_size = args.batch_size*2, num_workers=5, test_domain=args.test_domain, train_domain=args.train_domain, save_file=args.save_data_file_name)
else:
    loader = dataloader.dg_VLCS_dataloader(root=args.data_path, batch_size=args.batch_size, warmup_batch_size = args.batch_size*2, num_workers=5, test_domain=args.test_domain, train_domain=args.train_domain, save_file=args.save_data_file_name)
print('| Building Net')

model = get_model()
net1  = create_model()
net2  = create_model()
# net1  = create_algorithm_model()
# net2  = create_algorithm_model()
cudnn.benchmark = True

if args.swad:
    print("Using SWAD!!!!!")
    swad_net1 = swa_utils.AveragedModel(net1)
    swad_net2 = swa_utils.AveragedModel(net2)
    swad1 = LossValley(**hparams.swad_kwargs)
    swad2 = LossValley(**hparams.swad_kwargs)
    last_acc1, last_acc2 = 0, 0
...
model_name_1 = 'dg_vlcs_net1.pth.tar'
model_name_2 = 'dg_vlcs_net2.pth.tar'
...
for epoch in range(0, args.num_epochs+1):   
    ...
    if epoch<warm_up:             
        ...
        for i in range(nb_repeat):
            ...
            if args.swad:
                train(epoch, net1, net2, optimizer1, labeled_trainloader, unlabeled_trainloader, swad_net1)
                swad1.update_and_evaluate(swad_net1, last_acc1)
                acc1 = val(net1,val_loader,1, swad1)
                last_acc1 = acc1
            else:
        ...
        for i in range(nb_repeat):
            ...
            if args.swad:
                train(epoch,net2,net1,optimizer2,labeled_trainloader, unlabeled_trainloader, swad_net2)
                swad2.update_and_evaluate(swad_net2, last_acc2)
                acc2 = val(net2,val_loader,2, swad2)
                last_acc2 = acc2 
            else:
    ...
    if args.swad:
        acc, accs = test(net1,net2,test_loader, swad1, swad2) 
    else:
