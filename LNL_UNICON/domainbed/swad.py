...
class LossValley(SWADBase):
    ...
    def get_smooth_loss(self, idx):
        ...
        smooth_loss = max([model.end_acc for model in list(self.smooth_Q)[idx:]])
        return smooth_loss
    ...
    def update_and_evaluate(self, segment_swa, val_acc=0.0):
        if self.dead_valley:
            return

        frozen = copy.deepcopy(segment_swa.cpu())
        #frozen.end_loss = val_loss
        frozen.end_acc = val_acc
        ...
        if not self.is_converged:
            ...
            min_idx = np.argmax([model.end_acc for model in self.converge_Q])
            ...
            if min_idx == 0:
                ...
                th_base = np.mean([model.end_acc for model in self.converge_Q])
                ...
                if self.n_tolerance < self.n_converge:
                    ...
                    for i in reversed(range(len(Q))):
                        ...
                        if model.end_acc < self.threshold:
                            start_idx = i + 1
                            break
